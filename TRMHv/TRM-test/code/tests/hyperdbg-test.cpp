...
main(int argc, char * argv[])
{
    ...
    strcpy_s(Buffer, TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE, "Hey there, Are you TRMHv?");

    if (!strcmp(argv[1], "im-TRMHv"))
    {
        //
        // Perform our shaking with TRMHv
        //

        //
        // It's not called directly, it's probably from TRMHv
        //
        PipeHandle = NamedPipeClientCreatePipe("\\\\.\\Pipe\\TRMHvTests");
        ...
        if (strcmp(Buffer,
                   "Hello, Dear Test Process... Yes, I'm TRMHv Debugger :)") ==
            0)
        {
            //
            // *** Connected to the TRMHv debugger ***
            //

            //
            // Now we should request the test case number from the TRMHv Debugger
            //
            RtlZeroMemory(Buffer, TEST_CASE_MAXIMUM_BUFFERS_TO_COMMUNICATE);
