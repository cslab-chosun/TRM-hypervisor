...
KdSendStepPacketToDebuggee(DEBUGGER_REMOTE_STEPPING_REQUEST StepRequestType)
{
        ...
        StepRequestType == DEBUGGER_REMOTE_STEPPING_REQUEST_STEP_OVER_FOR_GU)
    {
        ...
        //
        if (TRMHvCheckWhetherTheCurrentInstructionIsCall(
...
KdSendPacketToDebuggee(const CHAR * Buffer, UINT32 Length, BOOLEAN SendEndOfBuffer)
{
    ...
    if (Length + SERIAL_END_OF_BUFFER_CHARS_COUNT > MaxSerialPacketSize)
    {
        ShowMessages("err, buffer is above the maximum buffer size that can be sent to debuggee (%d > %d), "
                     "for more information, please visit https://docs.TRMHv.org/tips-and-tricks/misc/increase-communication-buffer-size\n",
...
/**
 * @brief Sends a TRMHv packet to the debuggee
 *
    ...
    DEBUGGER_REMOTE_PACKET_REQUESTED_ACTION RequestedAction)
{
    ...
    //
    Packet.Indicator       = INDICATOR_OF_TRMHv_PACKET;
...
/**
 * @brief Sends a TRMHv packet + a buffer to the debuggee
 *
 * @param RequestedAction
 * @param Buffer
 * @param BufferLength
...
StartAgain:
    ...
    if (TheActualPacket->Indicator == INDICATOR_OF_TRMHv_PACKET)
    {
        //
        // Check checksum
        //
        ...
        default:
            ...
            Result = FALSE;
            ShowMessages("err, invalid packet is received. did you connect to an instance of TRMHv?\n"
    ...
    */
    ...
    if (IsPreparing)
    {
        ...
        if (!CommandLoadVmmModule())
        {
            CloseHandle(Comm);
            g_SerialRemoteComPortHandle    = NULL;
            g_IsConnectedToTRMHvLocally = FALSE;
        ...
        //
        // TRMHvInterpreter("!syscall script { print(@rax); }");
        // TRMHvInterpreter("!epthook fffff801`639b1030 script { print(@rax);
        // }"); TRMHvInterpreter("!msrwrite script { print(@rax); }");
        //
...
KdReloadSymbolsInDebuggee(BOOLEAN PauseDebuggee, UINT32 UserProcessId)
{
    ...
    // we choose the TRMHv's process to load its modules
    //
...
KdCloseConnection()
{
    ...
    {
        if (g_IsConnectedToTRMHvLocally && g_IsDebuggerModulesLoaded)
        {
            //
