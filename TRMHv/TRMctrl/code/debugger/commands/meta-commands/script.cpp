...
CommandScriptHelp()
{
    ShowMessages(".script : runs a TRMHv script.\n\n");
...
CommandScriptRunCommand(std::string Input, vector<string> PathAndArgs)
{
    ...
    CommandExecutionResult = TRMHvInterpreter(LineContent);
 ...
 */
VOID
TRMHvScriptReadFileAndExecuteCommand(std::vector<std::string> & PathAndArgs)
{
    std::string Line;
    ...
    if (File.is_open())
    {
        ...
        while (std::getline(File, Line))
        {
            ...
            //
            if (TRMHvCheckMultilineCommand((char *)Line.c_str(), Reset))
            {
                //
 ...
 */
int
TRMHvScriptReadFileAndExecuteCommandline(int argc, char * argv[])
{
    vector<string> Args;
    ...
    if (!Args.empty())
    {
        TRMHvScriptReadFileAndExecuteCommand(Args);
        printf("\n");
    }
    else
    {
...
CommandScript(vector<string> SplitCommand, string Command)
{
    ...
    */
    ...
    //
    TRMHvScriptReadFileAndExecuteCommand(PathAndArgs);
}
