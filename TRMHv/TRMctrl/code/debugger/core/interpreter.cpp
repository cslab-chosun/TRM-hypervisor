...
TRMHvInterpreter(char * Command)
{
    ...
    // still some of the commands should be handled in the local TRMHv
    //
...
TRMHvShowSignature()
{
    if (g_IsConnectedToRemoteDebuggee)
    {
        ...
        //
        ShowMessages("[%s:%s] TRMHv> ", g_ServerIp.c_str(), g_ServerPort.c_str());
    }
    ...
    else if (g_IsSerialConnectedToRemoteDebuggee)
    {
        ...
        //
        ShowMessages("%x: kTRMHv> ", g_CurrentRemoteCore);
    }
    else
    {
        //
 ...
 */
bool
TRMHvCheckMultilineCommand(char * CurrentCommand, bool Reset)
{
