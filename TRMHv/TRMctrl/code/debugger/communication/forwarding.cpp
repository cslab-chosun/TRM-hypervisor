                             ...
                             HMODULE *                      Module)
{
    ...
    else if (SourceType == EVENT_FORWARDING_MODULE)
    {
        ...
        TRMHv_event_forwarding_t TRMHv_event_forwarding = (TRMHv_event_forwarding_t)GetProcAddress(ModuleHandle, "TRMHv_event_forwarding");

        if (TRMHv_event_forwarding == NULL)
        {
            ShowMessages("err, unable to find the 'TRMHv_event_forwarding' function\n");
            return INVALID_HANDLE_VALUE;
        }
        ...
        *Module = ModuleHandle;
        ...
        //
        // The handle is the location of the TRMHv_event_forwarding function
        //
        return (void *)TRMHv_event_forwarding;
    }
                ...
                CurrentOutputSourceDetails->OutputUniqueTag)
            {
                    ...
                    EVENT_FORWARDING_STATE_OPENED)
                {
                    ...
                    case EVENT_FORWARDING_MODULE:
                        ((TRMHv_event_forwarding_t)CurrentOutputSourceDetails->Handle)(
                            Message,
                            MessageLength);
                        Result = TRUE;
                        break;
