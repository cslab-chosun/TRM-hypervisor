...
/**
 * @brief Sends a TRMHv response packet to the debugger
 *
 * @param PacketType
 * @param Response
...
/**
 * @brief Sends a TRMHv logging response packet to the debugger
 *
 * @param OptionalBuffer
 * @param OptionalBufferLength
 * @param OperationCode
...
KdCheckUserModePriorityBuffers()
{
    ...
    if (LogCallbackCheckIfBufferIsFull(TRUE))
    {
        LogWarning("Warning, the user-mode priority buffers are full, thus the new action replaces "
                   ...
                   "For more information please visit: https://docs.TRMHv.org/tips-and-tricks/misc/instant-events\n");
    }
}
...
{
    //
    // During the debugging of TRMHv, we realized that whenever an
    ...
    // is not big enough to handle TRMHv's command dispatching
...
KdDispatchAndPerformCommandsFromDebugger(PROCESSOR_DEBUGGING_STATE * DbgState)
{
    ...
    while (TRUE)
    {
        ...
        {
            //
            // It's not a TRMHv packet, the packet is probably deformed
            //
            LogError("Err, it's not a TRMHv packet, the packet is probably deformed\n");
            continue;
        }
...
KdHaltSystem(PDEBUGGER_PAUSE_PACKET_RECEIVED PausePacket)
{
    ...
    // of their execution codes and not on TRMHv routines
    //
    // KdBroadcastHaltOnAllCores();
    //
