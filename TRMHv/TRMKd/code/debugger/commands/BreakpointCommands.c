...
BreakpointCheckAndPerformActionsOnTrapFlags(UINT32 ProcessId, UINT32 ThreadId, BOOLEAN * TrapSetByDebugger)
{
    ...
    *TrapSetByDebugger = Result;
    ...
    else if (!Result && Rflags.TrapFlag)
    {
        ...
        //         "which was not previously adjusted by TRMHv. This occurrence could indicate "
        //         "the employment of an anti-debugging technique by the process or the involvement "
        //         "of another debugger. By default, TRMHv automatically manages these #DB events "
...
Return:
    ...
    //
    // By default, #DBs are managed by TRMHv
    //
...
BreakpointCheckAndHandleDebugBreakpoint(UINT32 CoreId)
{
                                                    ...
                                                    &TrapSetByDebugger))
    {
        ...
        else
        {
            ...
            // TRMHv but, we couldn't find any routines that gonna
    ...
    else
    {
        ...
        // By default TRMHv intercepts all #DBs and break the debugger if
...
BreakpointHandleBreakpoints(UINT32 CoreId)
{
    ...
    if (g_KernelDebuggerState)
    {
        ...
        {
            //
            // To avoid the computer crash situation from the TRMHv's breakpoint hitting while the interception is on
            ...
            {
                //
                // re-inject back to the guest as not handled if the interception is on and the breakpoint is not from the TRMHv's breakpoints
                //
                return FALSE;
            }
...
BreakpointAddNew(PDEBUGGEE_BP_PACKET BpDescriptorArg)
{
    ...
    // then the TRMHv simply ignores the breakpoints but it makes the computer slow
